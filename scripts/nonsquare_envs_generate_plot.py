...
# Read the CSV file
df = pd.read_csv('/home/hayden/code/csci312/vacuum-agents/nonsquare_envs_results.csv')
...
# Extract "nonsquare_nxm" from the environment names
df['environment'] = df['environment'].str.extract(r'nonsquare_(\d+x\d+)')

# Create a 1x2 grid of subplots
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(14, 6))

# Subplot 1: Score vs. Environment
axes[0].set_xlabel('Environment Dimensions')
axes[0].set_ylabel('Score')

# Subplot 2: Time Steps vs. Environment
axes[1].set_xlabel('Environment Dimensions')
...
plt.suptitle('Agent Performance In Non-Square Environments')
